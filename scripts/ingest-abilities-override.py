...
STAT_FALLBACKS = {"riposte": "windup", "thwack": "release"}


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "-i", "--input_json", required=True, help="Path to the input JSON file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--changelog_location",
        required=True,
        help="Path to output the changelog json",
    )
    ...
    for name, item in data.items():
        process_item(
            name, item, base_defaults, attack_defaults, weapon_defaults, weapons
        )
...
def process_item(name, item, base_defaults, attack_defaults, weapon_defaults, weapons):
    ...
    if name_parts[0] == "Default":
        process_default_item(
            name_parts, attack_type, item, base_defaults, attack_defaults
        )
    else:
...
def process_weapon_item(name_parts, attack_type, item, weapon_defaults, weapons):
    if len(name_parts) == 1:
        ...
        weapons[weapon_name]["attacks"] = process_attack(
            attack_type, item, weapons[weapon_name]["attacks"]
        )
    ...
    try:
        ...
        for name, changes in changelog.items():
            for change in changes:
                ...
                print(
                    full_path + ": " + str(change["old"]) + " -> " + str(change["new"])
                )
        ...
        for name, changes in changelog.items():
            ...
            for change in changes:
                new_string = (
                    ...
                    else json.dumps(change["new"], indent=12)
                )
                changelog_text += (
                    "\t"
                    + ".".join(change["path"])
    ...
    for key in dict2:
        ...
        if key in dict1:
            if isinstance(dict1[key], Mapping) and isinstance(value, Mapping):
                (new_changes, _) = deep_merge(
                    name, dict1[key], value, path + [str(key)]
                )
                changes += new_changes
            else:
                if dict1[key] == value:
                    ...
                    changes.append(
                        {"path": path + [key], "old": dict1[key], "new": value}
                    )
                    dict1[key] = value
        else:
